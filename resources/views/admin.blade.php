...
<body>
    ...
    <header>
      <div id="navbar">
      <div class="a1"> 
          <a href="{{ route('admin.dashboard') }}" class="button-link">LemonAid Study</a>
      </div>
            ...
            <td>
              @if (is_null($submission->is_correct))
                <form action="{{ route('admin.submissions.approve', $submission->id) }}" method="POST" style="display:inline;">
                  @csrf
                  <button type="submit" name="approve" value="1">Mark Correct</button>
                </form>
                <form action="{{ route('admin.submissions.approve', $submission->id) }}" method="POST" style="display:inline;">
                  @csrf
                  <button type="submit" name="approve" value="0">Mark Incorrect</button>
                </form>
              @else
                No action
              @endif
